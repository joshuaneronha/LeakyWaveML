...
def import_data():
    ...
    timestamp_list = list(set([x.split('.')[0] for x in os.listdir('comsol_results/3grey/')]))
    ...
    for timestamp in timestamp_list:
        ...
        with open('comsol_results/3grey/' + timestamp + '.csv', 'rb') as file:
        ...
        with open('comsol_results/3grey/' + timestamp + '.pkl', 'rb') as file:
...
def import_data_bin():
    ...
    for timestamp in timestamp_list:
        ...
        with open('comsol_results/1dconstantslots/' + timestamp + '.csv', 'rb') as file:
            results = np.loadtxt(file, delimiter=",", dtype=float)
            num_sims = int(results.shape[0] / 361)
            points = [361 * x for x in np.arange(num_sims + 1)]
            sorted_x = np.array([20*np.log10(results[i:i + 361,1]) for i in points[:-1]])
        ...
        with open('comsol_results/1dconstantslots/' + timestamp + '.pkl', 'rb') as file:
            slots = np.array(pickle.load(file))
            # print(slots.shape)
            slots_list.append(slots)
