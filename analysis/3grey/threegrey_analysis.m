...
Files=dir('gray-june29-2');
N = length(Files);
[t,w]=textread(strcat('gray-june29-2/',Files(3).name),'%f%f','headerlines',6);  % read first for size
...
for k = 3:N
    name = Files(k).name;
    x = split(name,'.');
    [T(:,str2double(x(1)) + 1),W(:,str2double(x(1)) + 1)]=textread(strcat('gray-june29-2/',name),'%f%f','headerlines',6);
...
objective241 = readmatrix('objective34.csv');
expected241 = readmatrix('data34.csv');
...
%% plotting
figure
polarplot(deg2rad(1:149),mag2db(abs(B(2112,3:151)) / max(abs(B(2112,3:151)))),'Color','#f7a400','LineWidth',2)
hold on
...
thetalim([0 180])
thetaticks(0:30:180)
legend('Experimental','Objective','Predicted','location','southwest')
set(gcf,'color','w');
