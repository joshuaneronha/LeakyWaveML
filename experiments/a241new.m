...
Files=dir('binarywaveguide241new');
N = length(Files);
[t,w]=textread(strcat('binarywaveguide241new/',Files(3).name),'%f%f','headerlines',6);  % read first for size
...
for k = 3:N
    name = Files(k).name;
    x = split(name,'.');
    [T(:,str2double(x(1)) + 1),W(:,str2double(x(1)) + 1)]=textread(strcat('binarywaveguide241new/',name),'%f%f','headerlines',6);
...
% figure
polarplot(deg2rad(13:151),mag2db(abs(B(2112,1:end-13)) / max(abs(B(2112,1:end-13)))),'Color','#0c2c84','LineWidth',2)
hold on
