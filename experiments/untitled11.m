...
Files=dir('3grey');
N = length(Files);
[t,w]=textread(strcat('3grey/',Files(3).name),'%f%f','headerlines',6);  % read first for size
...
for k = 3:N
    name = Files(k).name;
    x = split(name,'.');
    [T(:,str2double(x(1)) + 1),W(:,str2double(x(1)) + 1)]=textread(strcat('3grey/',name),'%f%f','headerlines',6);
...
expected241 = readmatrix('3greytest.csv');
% expected241cont = readmatrix('width-modulation-control.csv');
