...
Files=dir('binarywaveguide477-cutback');
N = length(Files);
[t,w]=textread(strcat('binarywaveguide477-cutback/',Files(3).name),'%f%f','headerlines',6);  % read first for size
...
for k = 3:N
    name = Files(k).name;
    x = split(name,'.');
    [T(:,str2double(x(1)) + 1),W(:,str2double(x(1)) + 1)]=textread(strcat('binarywaveguide477-cutback/',name),'%f%f','headerlines',6);
