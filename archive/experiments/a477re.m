...
Files=dir('binarywaveguide477-realigned');
N = length(Files);
[t,w]=textread(strcat('binarywaveguide477-realigned/',Files(3).name),'%f%f','headerlines',6);  % read first for size
...
for k = 3:N
    name = Files(k).name;
    x = split(name,'.');
    [T(:,str2double(x(1)) + 1),W(:,str2double(x(1)) + 1)]=textread(strcat('binarywaveguide477-realigned/',name),'%f%f','headerlines',6);
...
figure
polarplot(deg2rad(11:151),mag2db(abs(B(2112,1:end-11)) / max(abs(B(2112,1:end-11)))),'Color','#0c2c84','LineWidth',2)
hold on
polarplot(deg2rad(1:151),mag2db(objective241(1:151,2) / max(objective241(1:151,2))),'Color','#3c7dce','LineWidth',2)
polarplot(deg2rad(1:151),mag2db(expected241(1:151,2) / max(expected241(1:151,2))),'Color','#7fcdbb','LineWidth',2)
