...
Files=dir('greywaveguide/');
N = length(Files);
[t,w]=textread(strcat('greywaveguide/',Files(4).name),'%f%f','headerlines',6);  % read first for size
...
for k = 4:N
    name = Files(k).name;
    x = split(name,'.');
    [T(:,str2double(x(1)) + 1),W(:,str2double(x(1)) + 1)]=textread(strcat('greywaveguide/',name),'%f%f','headerlines',6);
...
expected241 = readmatrix('expected71grey.csv');
random = readmatrix('expected432grey.csv');
...
%% plotting
figure
polarplot(deg2rad(1:151),mag2db(abs(B(2112,1:151)) / max(abs(B(2112,1:151)))),'Color','#0c2c84','LineWidth',2)
hold on
% polarplot(deg2rad(1:151),mag2db(objective241(1:151,2) / max(objective241(1:151,2))),'Color','r','LineWidth',2)

polarplot(deg2rad(1:151),mag2db(expected241(1:151,1) / max(expected241(1:151,1))),'Color','r','LineWidth',2)
...
% 
hold on
% polarplot(deg2rad(1:151),mag2db(abs(B(2111,1:151)) / max(abs(B(2111,1:151)))),'Color','b','LineWidth',2)
% polarplot(deg2rad(1:151),mag2db(abs(B(2095,1:151)) / max(abs(B(2095,1:151)))),'Color','g','LineWidth',2)
ax = gca;
pax.RAxisLocation   =   0;
...
% end
% 
% polarplot(deg2rad(4:151),mag2db(abs(B(2112,1:end-4)) / max(abs(B(2112,1:end-4)))),'Color','red','LineWidth',2)
% %%loc
% [pks_manual,locs_manual] = findpeaks(mag2db(abs(B(2112,1:151-14)) / max(abs(B(2112,1:151-14)))),deg2rad(15:151),'SortStr','descend');
% [pks_shift, locs_shift] = findpeaks(mag2db(abs(B(2112,1:151)) / max(abs(B(2112,1:151)))),real(solvedthetap(1:151)),'SortStr','descend');
% [pks_obj, locs_obj] = findpeaks(mag2db(objective241(1:151,2) / max(objective241(1:151,2))),deg2rad(1:151),'SortStr','descend');